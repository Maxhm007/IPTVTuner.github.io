...
USER_AGENT="Mozilla/5.0 (IPTVTuner-SelfHeal/3.2)"
TIMEOUT=6
SOURCE_TIMEOUT=12
...
def fetch_bytes(url,timeout=TIMEOUT,max_bytes=524288,range_request=False):
    headers={"User-Agent":USER_AGENT,"Accept":"application/vnd.apple.mpegurl, application/x-mpegURL, video/*, audio/*, */*"}
    if range_request: headers["Range"]=f"bytes=0-{max_bytes-1}"
    with urllib.request.urlopen(urllib.request.Request(url,headers=headers),timeout=timeout) as r:
        return r.read(max_bytes),getattr(r,"url",url)

def fetch_text(url,timeout=TIMEOUT):
    b,u=fetch_bytes(url,timeout)
    return b.decode("utf-8",errors="replace"),u
...
def parse_playlist(text):
    ...
    while i<len(lines):
        ...
        if s.startswith(INACTIVE_INFO+"#EXTINF:"):
            inactive=True; info=s[len(INACTIVE_INFO):]
        ...
        if inactive:
            ...
            if j>=len(lines) or not lines[j].strip().startswith(("http://","https://")):
                i+=1; continue
            url=lines[j].strip()
        out.append({"info":info,"url":url,"name":attrs.get("tvg-name") or name,"display":name,"tvg_id":attrs.get("tvg-id",""),"group":attrs.get("group-title",""),"inactive":inactive,"raw_info":raw,"raw_url":lines[j]})
        i=j+1
    return out
...
def sync_legacy_catalog(p):
    ...
    existing={catalog_key(e) for e in parse_playlist(cur)}
    add=[]; skipped=0
    ...
    print(f"legacy sync: imported={len(add)}, skipped={skipped}")
    return len(add)
...
def load_candidates():
    ...
    for name,url,rank in SOURCES:
        try:
            text,_=fetch_text(url,SOURCE_TIMEOUT)
            parsed=parse_playlist(text)
            for e in parsed:
                e["source"]=name; e["source_rank"]=rank
            out+=parsed
            print(f"loaded {name}: {len(parsed)}")
        except Exception as ex:
            print(f"warning source {name}: {ex}")
    return out
...
def same_channel(a,b):
    ...
    ag,bg=normalize(a.get("group")),normalize(b.get("group"))
    return not(ag and bg and ag!=bg)

def find_recovery(e,cands,used,pending=None):
    trial=[]
    # Original URL gets first chance to recover.
    if e.get("url"):
        trial.append({"url":e["url"],"source":"original-recovery","source_rank":-1})
    if pending and pending!=e.get("url"):
        trial.append({"url":pending,"source":"pending-recovery","source_rank":0})
    if not is_restricted(e):
        matches=[c for c in cands if c["url"]!=e.get("url") and same_channel(e,c) and not credential_style(c["url"])]
        matches.sort(key=lambda c:(c.get("source_rank",99),not c["url"].startswith("https://")))
        seen={x["url"] for x in trial}
        for c in matches:
            if c["url"] not in seen:
                trial.append(c); seen.add(c["url"])
    seen=set()
    for c in trial[:MAX_REPLACEMENT_CANDIDATES+1]:
        u=c["url"]
        if u in seen: continue
        seen.add(u)
        owner=used.get(u)
        if owner and owner!=catalog_key(e): continue
        print(f"  validating recovery from {c.get('source','source')}: {u}")
        if is_working_hls(u,2): return u,c.get("source","unknown")
    return None,None

def replace_block(text,e,info,url):
    return text.replace(e["raw_info"]+"\n"+e["raw_url"],info+"\n"+url,1)
...
def load_state(path):
    ...
    d.setdefault("version",3); d.setdefault("cursor",0); d.setdefault("channels",{})
    return d

def channel_state(state,e):
    k=catalog_key(e); r=state["channels"].setdefault(k,{})
    for a,v in (("name",e["display"]),("group",e.get("group","")),("failures",0),("recovery_successes",0),("pending_url",""),("last_working_url",""),("status","inactive" if e["inactive"] else "active")):
        r.setdefault(a,v)
    return k,r

def parallel_health(entries,indexes):
    result={}
    active=[i for i in indexes if not entries[i]["inactive"]]
...
def write_report(path,entries,state,checked,imported,repl,off,on):
    active=sum(not e["inactive"] for e in entries)
    lines=["# IPTV V008 Health","",f"- Updated: {now_iso()}",f"- Total channels: {len(entries)}",f"- Active: {active}",f"- Inactive: {len(entries)-active}",f"- Checked this run: {checked}",f"- Imported from V007: {imported}",f"- Replaced: {len(repl)}",f"- Inactivated: {len(off)}",f"- Reactivated: {len(on)}","","## Inactive channels",""]
    dead=[e for e in entries if e["inactive"]]
    for e in dead:
        r=state["channels"].get(catalog_key(e),{})
        lines.append(f"- {e['display']} — recovery={r.get('recovery_successes',0)}/{RECOVERY_SUCCESSES_REQUIRED}")
    if not dead: lines.append("- None")
    path.write_text("\n".join(lines)+"\n",encoding="utf-8")

def heal(playlist,state_dir=None):
    p=Path(playlist)
    root=Path(state_dir) if state_dir else p.parent
    root.mkdir(parents=True,exist_ok=True)
    sp=root/"IPTV-V008-health.json"
    rp=root/"IPTV-V008-health.md"
    imported=sync_legacy_catalog(p)
    original=p.read_text(encoding="utf-8")
    entries=parse_playlist(original)
    state=load_state(sp)
    valid={catalog_key(e) for e in entries}
    state["channels"]={k:v for k,v in state["channels"].items() if k in valid}

    # Full sweep every run so dead links are removed from the active playlist immediately.
    ...
    for idx in indexes:
        e=entries[idx]
        prefix=f"[{idx+1}/{len(entries)}]"
        k,r=channel_state(state,e)
        r["last_checked"]=now_iso()

        if e["inactive"]:
            print(f"{prefix} INACTIVE {e['display']}: recovery search")
            if cands is None: cands=load_candidates()
            u,src=find_recovery(e,cands,used,r.get("pending_url") or None)
            if u:
                r["recovery_successes"]=(int(r.get("recovery_successes",0))+1) if r.get("pending_url")==u else 1
                r["pending_url"]=u
                r["replacement_source"]=src
                if r["recovery_successes"]>=RECOVERY_SUCCESSES_REQUIRED:
                    original=replace_block(original,e,e["info"],u)
                    used[u]=k
                    r.update({"status":"active","failures":0,"recovery_successes":0,"pending_url":"","last_working_url":u})
                    on.append((e["display"],u))
                    print(f"  REACTIVATED -> {u}")
                else:
                    print(f"  recovery candidate passed ({r['recovery_successes']}/{RECOVERY_SUCCESSES_REQUIRED})")
            else:
                r["recovery_successes"]=0; r["pending_url"]=""
                print("  still inactive")
            continue

        print(f"{prefix} checking {e['display']}: {e['url']}")
        if health.get(idx,False):
            r.update({"status":"active","failures":0,"recovery_successes":0,"pending_url":"","last_working_url":e["url"]})
            print("  OK (manifest + segment)")
            continue

        # Fail-fast policy: remove a non-working stream from the active M3U immediately.
        original=replace_block(original,e,INACTIVE_INFO+e["info"],INACTIVE_URL+e["url"])
        r.update({"status":"inactive","failures":int(r.get("failures",0))+1,"pending_url":"","recovery_successes":0,"last_failure":now_iso()})
        off.append((e["display"],e["url"]))
        print("  FAILED -> INACTIVATED immediately")

    state.update({"version":3,"cursor":0,"last_run":now_iso(),"last_run_checked":len(indexes),"total_channels":len(entries)})
    sp.write_text(json.dumps(state,indent=2,sort_keys=True)+"\n",encoding="utf-8")
    if imported or repl or off or on:
        p.write_text(original,encoding="utf-8")
    latest=parse_playlist(original)
    write_report(rp,latest,state,len(indexes),imported,repl,off,on)
    print(f"run summary: checked={len(indexes)}, replaced={len(repl)}, inactivated={len(off)}, reactivated={len(on)}")
